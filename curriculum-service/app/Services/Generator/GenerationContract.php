...
class GenerationContract
{
    ...
    /** @return array{max_sub_cpmk:int,total_weeks:int,learning_weeks:int,pola:string,pekan_pengantar:int,pekan_uts:?int,pekan_uas:?int} */
    public function limits(?MataKuliah $mk): array
    {
        // Batas pengaman, bukan target: MK padat boleh melebihi jumlah pekan dan
        // menumpuk beberapa Sub-CPMK pada satu pekan.
        $batas = max(1, (int) config('generator.max_sub_cpmk', 40));
        ...
        $evaluasi = $this->estimasi->pekanEvaluasi($mk->institusi_id, $n);
        $learning = $pola === 'reguler'
            ? max(0, $n - count(array_unique($evaluasi)))
            : $n;
        ...
        return [
            ...
            'pola' => $pola,
            'pekan_pengantar' => 1,
            'pekan_uts' => $pola === 'reguler' ? $evaluasi['uts'] : null,
            'pekan_uas' => $pola === 'reguler' ? $evaluasi['uas'] : null,
        ];
    }
    ...
    public function systemDirective(?MataKuliah $mk): string
    {
        ...
        $target = $l['pola'] === 'reguler'
            ? "Pola reguler: jumlah Sub-CPMK = minimum yang cukup untuk skop MK, berkode urut Sub-CPMK-1 dst; acuan nyaman ≈ {$l['learning_weeks']} (satu per pekan belajar). Boleh LEBIH bila substansi menuntut — kelebihannya dipetakan sebagai Sub-CPMK tambahan pada pekan yang sama, bukan menambah pekan. UTS/UAS bukan Sub-CPMK."
            : "Pola {$l['pola']}: jumlah Sub-CPMK minimum yang cukup, batas pengaman {$l['max_sub_cpmk']}.";
        $struktur = $l['pola'] === 'reguler'
            ? "- Struktur pekan reguler BAKU: {$l['total_weeks']} pekan; pekan {$l['pekan_pengantar']} = pengantar/orientasi MK (kontrak kuliah & ruang lingkup, boleh sekaligus Sub-CPMK pertama), pekan {$l['pekan_uts']} = UTS, pekan {$l['pekan_uas']} = UAS. Jangan menggeser atau menambah pekan.\n"
            : '';
        return "\nKONTRAK SERVER TETAP (mengatasi instruksi template/konteks yang bertentangan):\n"
            . "- Batas pengaman TOTAL Sub-CPMK & CPMK = {$l['max_sub_cpmk']} — itu batas atas, bukan target. Jangan memotong hasil diam-diam.\n"
            . "{$target}\n"
            . $struktur
            ...
            . "- Reguler: satu BARIS per pekan. Sub-CPMK utama dipetakan berurutan; bila Sub-CPMK lebih banyak daripada pekan belajar, sisanya ditaruh pada `sub_cpmk_kode_tambahan` pekan yang paling relevan. Pekan ujian memakai sub_cpmk_kode null dan mengevaluasi kemampuan yang telah diajarkan (boleh mencantumkan Sub-CPMK yang diuji pada sub_cpmk_kode_tambahan), bukan konsep baru. Blok/profesi boleh beberapa baris/pertemuan per pekan.\n"
    ...
    public function userDirective(?MataKuliah $mk): string
    {
        $l = $this->limits($mk);
        $sub = $l['pola'] === 'reguler'
            ? "Susun Sub-CPMK berkode urut Sub-CPMK-1 dst sejumlah minimum yang cukup (acuan ≈ {$l['learning_weeks']}, boleh lebih bila substansi menuntut); UTS/UAS bukan Sub-CPMK dan tanpa nomor minggu. Pekan {$l['pekan_pengantar']} = pengantar MK, pekan {$l['pekan_uts']} = UTS, pekan {$l['pekan_uas']} = UAS."
            : "Susun Sub-CPMK sejumlah minimum yang cukup, batas pengaman {$l['max_sub_cpmk']}.";
        return "PARAMETER CAPAIAN MK: pola {$l['pola']}, {$l['total_weeks']} pekan, {$l['learning_weeks']} pekan belajar; batas pengaman {$l['max_sub_cpmk']} Sub-CPMK dan {$l['max_sub_cpmk']} CPMK. "
    ...
    public function violations(string $stage, array $data, GenerateSession $session, ?MataKuliah $mk, bool $strict = true): array
    {
        ...
        if (in_array($stage, ['cpmk', 'sub_cpmk'], true) && count($rows) > $l['max_sub_cpmk']) {
            $errors[] = "{$stage}: batas pengaman {$l['max_sub_cpmk']} butir TOTAL terlampaui, diterima " . count($rows) . '. Kurangi melalui suntingan; hasil tidak dipotong otomatis.';
        }
        ...
        foreach ($rows as $i => $row) {
            ...
            // Sub-CPMK lain yang juga disasar pekan ini ikut menghitung cakupan.
            if ($stage === 'mingguan') {
                $tambahan = $row['sub_cpmk_kode_tambahan'] ?? null;
                if ($tambahan !== null && ! $this->textList($tambahan)) {
                    $errors[] = "{$path}.sub_cpmk_kode_tambahan harus daftar kode unik.";
                } else {
                    foreach ((array) $tambahan as $code) {
                        if (! in_array($code, $parents, true)) $errors[] = "{$path}.sub_cpmk_kode_tambahan: kode induk tidak dikenal {$code}.";
                        else $covered[] = $code;
                    }
                }
            }
